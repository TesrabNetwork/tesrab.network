...
An exchange is not decentralized if it can lose, freeze or destroy customer funds OR if it can freeze customer funds. If it can do BOTH. It's a false sense of decentralization. The fact the DEX teams have the ability to freeze funds with a built-in kill switch that's an exclusively centrally controlled exchange feature is not only worrisome but forces uses to  
